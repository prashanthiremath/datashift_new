  ...
  class DocContext
    ...
    def save_and_monitor_progress
       ...
       # @progress_monitor.failure(failed)
        ...
        if save
          ...
          puts ("Successfully Processed [#{node_context.method_binding.spp}]")
          puts ("Successfully SAVED Object #{@progress_monitor.success_inbound_count} - [#{load_object.id}]")
        else

          failed = FailureData.new(load_object, node_context, current_errors)
          puts "ERROR WHY"
          puts load_object.valid?
          puts load_object.errors.full_messages 

          puts ("Failed to Process [#{node_context.method_binding.spp}]")
          puts ("Failed to SAVE Object #{@progress_monitor.success_inbound_count} - [#{load_object.inspect}]")
        end
      #end
    end
    ...
    def save
      return false unless  load_object
       ...
       puts load_object.errors.full_messages
       puts ("SAVING #{load_object.class} : #{load_object.inspect}")
      begin
        load_object.save!
        puts "ERRPR LOAD OBJECT"
        puts load_object.inspect
        puts load_object.valid?
        puts load_object.errors.full_messages.inspect
      rescue StandardError => e

        puts( "Save Error : #{e.inspect} on #{load_object.class}")
        puts(e.backtrace)
        false
      end
    end
